...
async def extract_questions(
    ...
    URL = f"https://{subdomain}.{domain}/{api_version}/questions?order={order}&sort={sort_by}&min={min}&fromdate={str_to__midnight_utc(str_date=from_date)}&todate={str_to__midnight_utc(str_date=to_date)}&site={site}&tagged={tag_string}&page={page}&pagesize={pagesize}&filter=withbody&key={stackapps_key}&access_token={stackapps_token}"
...
async def extract_answers_by_question_id(
    ...
    URL = f"https://{subdomain}.{domain}/{api_version}/questions/{ids_string}/answers?order={order}&pagesize={pagesize}&sort={sort_by}&site=stackoverflow&filter=withbody&key={stackapps_key}&access_token={stackapps_token}"
    async with httpx.AsyncClient() as client:
        data = await fetch(client, URL)
...
async def extract_answers_by_answer_id(
    ...
    for id in ids:
        ids_string = ids_string + f'{id};'
    ids_string = ids_string[:-1]
    URL = f"https://{subdomain}.{domain}/{api_version}/answers/{ids_string}?order={order}&sort={sort_by}&pagesize={pagesize}&site=stackoverflow&filter=withbody&key={stackapps_key}&access_token={stackapps_token}"
    async with httpx.AsyncClient() as client:
        data = await fetch(client, URL)
...
def get_question_answer_pairs(from_date, to_date, tags, stackapps_token, stackapps_key):
    """
    ...
    questions = asyncio.run(
        extract_questions(
            ...
            pagesize=100, 
            page=1,
            stackapps_token=stackapps_token,
            stackapps_key=stackapps_key
            )
        )
